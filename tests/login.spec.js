...
test.describe("Login", () => {
  ...
  test('Email allows alphabetic characters', async ({ page }) => {
    ...
    await expect(page.getByRole("textbox", { name: "Email Address" })).toHaveValue("admin");
  });
  // Nhập email là số 
  test('Email allows number characters', async ({ page }) => {
    await page.goto('https://crm.anhtester.com/admin/authentication');
    await page.getByRole("textbox", { name: "Email Address" }).fill("123456");
    await expect(page.getByRole("textbox", { name: "Email Address" })).toHaveValue("123456");
  });
  ...
  test('Email with spaces', async ({ page }) => {
    ...
    await expect(page.getByRole("textbox", { name: "Email Address" })).toHaveValue("admin@example.com");
  });
  ...
  // Nhập chữ 
  test('Password allows alphabetic characters and is masked', async ({ page }) => {
    await page.goto('https://crm.anhtester.com/admin/authentication');
    await page.getByRole("textbox", { name: "Password" }).fill("password");
    await expect(page.getByRole("textbox", { name: "Password" })).toHaveValue("password");    // Verify password được nhập đúng
    await expect(page.getByRole("textbox", { name: "Password" })).toHaveAttribute('type', 'password');    // Verify password được mask
  });
  // Nhập số 
  ...
  test('Password with spaces and is masked', async ({ page }) => {
    ...
    await expect(page.getByRole("textbox", { name: "Password" })).toHaveValue("pass word");
    await expect(page.getByRole("textbox", { name: "Password" })).toHaveAttribute('type', 'password');
  });
  ...
  test('Password with spaces at the beginning and end and is masked', async ({ page }) => {
    await page.goto('https://crm.anhtester.com/admin/authentication');
    await page.getByRole("textbox", { name: "Password" }).fill("  password  ");
